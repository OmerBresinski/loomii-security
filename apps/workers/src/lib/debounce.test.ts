...
describe("Debouncing", () => {
  ...
  it("uses consistent jobId for same entity", async () => {
    ...
    expect(opts.jobId).toBe("assemble:tenant_1:LOO-123");
  });

  it("sets 60s delay on assembly job", async () => {
    const payload = {
      eventId: "evt_1",
      tenantId: "tenant_1",
  ...
  it("different entities get different jobIds", async () => {
    ...
    expect(opts1.jobId).toBe("assemble:tenant_1:LOO-100");
    expect(opts2.jobId).toBe("assemble:tenant_1:LOO-200");
  ...
  it("different tenants for same entity get different jobIds", async () => {
    ...
    expect(opts1.jobId).toBe("assemble:tenant_A:LOO-100");
    expect(opts2.jobId).toBe("assemble:tenant_B:LOO-100");
  });
  ...
  describe("getDebounceJobId", () => {
    it("generates correct format", () => {
      expect(getDebounceJobId("tenant_1", "LOO-123")).toBe("assemble:tenant_1:LOO-123");
    });

    it("handles Notion page IDs", () => {
      expect(getDebounceJobId("tenant_1", "abc123-def456")).toBe(
        "assemble:tenant_1:abc123-def456"
      );
    });
  });
